...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['weekR_Perfect', 'weekt_Perfect', 'secret']);
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'weekR_Perfect' | 'weekT_Perfect':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice && ratingPercent >= 1)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'weekR':
									if(achievementName == 'weekR_Perfect') unlock = true;
								case 'weekT':
									if(achievementName == 'weekT_Perfect') unlock = true;

							}
						}
					case 'secret':
						if(Paths.formatToSongPath(SONG.song) == 'Remorseless') {
							unlock = true;
						}
				}
