function onCreate()
	for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'SkillStarNoteTaptroupe' then
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
    if noteType == 'SkillStarNoteTaptroupe' then
        if getPropertyFromGroup('notes', id, 'rating') == 'sick' then
            ...
            addScore(500)
            characterPlayAnim('boyfriend', 'TAAPRIGHT', true);
...
function onEvent(name, value1, value2)
    if name == 'SkillStartMoment' then
        ...
        setProperty('littlestar.x',1073)
        setProperty('littlestar.y',512)
    end
end